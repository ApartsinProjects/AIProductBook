...
"Would a thoughtful reader, after reading this section, understand not just WHAT to do but WHY it works?"
...
- Missing justification: A concept is introduced without explaining why it was needed
- Unmotivated definitions: A term is defined without first showing why we need it
- Solutions before problems: The text describes how something works before explaining why we need it

For each violation, draft a 2 to 4 sentence problem statement that should precede the concept.
...
### Missing Intuition
- Technical concepts without intuitive explanation
- Processes described without explaining the reasoning behind the design
- Architecture choices presented without explaining what was rejected and why

### Shallow Explanations
- "Use tool X to do Y" without explaining what X does internally
...
When deepening explanations, check whether the concept connects to material in other chapters. Add inline cross-reference links where appropriate.

## Example Issues

- "The text says 'RAG uses retrieval to ground responses' but never explains WHY grounding matters (hallucination problem) or HOW it solves it."
- "Evaluation metrics are presented but no intuition is given for why accuracy alone fails for AI products (task completion vs accuracy)."
- "The claim 'eval-first development is essential' is stated without evidence. Add: Without evals, teams waste months building features they cannot measure."
...
1. [Concept] in [chapter/section]
   - Current: [how it is introduced now]
   - Missing: [the problem/motivation that should come first]
   - Fix: [draft the 2-4 sentence problem statement]
   - Priority: HIGH / MEDIUM / LOW

### Unjustified Claims
1. [Claim] in [chapter/section]
...
### Summary
[Overall depth assessment: STRONG / ADEQUATE / NEEDS WORK]
```