...
The book uses a standard structure:
```
book-root/
...
2. **Part indexes** (`part-*/index.html`): Lists every chapter directory that exists in that part, with correct titles and links. Section counts per chapter are accurate.
3. **Chapter indexes** (`part-*/chapter-*/index.html`): Lists every `section-*.html` file in the chapter directory. Section titles match the `<h1>` or `<title>` in each section file. Section descriptions are present.
...
This 3-level hierarchy should be consistent throughout the book.
