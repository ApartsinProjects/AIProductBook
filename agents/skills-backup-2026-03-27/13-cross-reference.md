...
Books are organized as: Parts > Chapters > Sections

Each part contains multiple chapters, and each chapter contains multiple sections.
...
- **Contextual bridge**: Linking a concept to where it appears in a different context (e.g., linking "loss function" in a foundations chapter to where it appears in advanced topics)

## Relative Path Rules

Links use relative paths following the book's folder structure:
- Chapters are organized under part directories
- Use relative paths to link between chapters following the pattern: `../../part-N-name/chapter-XX-name/index.html` or similar
...
- Never link the same target chapter more than twice in one section
