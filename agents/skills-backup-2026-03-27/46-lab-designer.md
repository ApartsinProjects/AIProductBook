...
The following are example lab ideas organized by topic area. Adapt these to fit the specific book content.

- **Foundational Topics**: Build a tokenizer from scratch, implement attention step by step, create a text generator with different decoding strategies
- **Understanding Models Topics**: Reproduce a scaling law plot, benchmark quantized vs full models, profile inference latency
- **Working with Models Topics**: Build a multi-provider API wrapper, create a prompt optimization pipeline, build a hybrid ML+LLM classifier
- **Training Topics**: Generate and filter synthetic data, fine-tune a small model on custom data, run LoRA on a local model, interpret model internals with probing
- **Retrieval Topics**: Build a RAG pipeline end-to-end, create a conversational agent with memory, compare embedding models
- **Agent Topics**: Build a ReAct agent from scratch, create a multi-agent debate system, set up an evaluation harness
- **Production Topics**: Build a safety filter pipeline, create a cost calculator for model deployment decisions
